...
class BidSniper:
    def outage_check_hook(self, http_response: Response, *args, **kwargs):
        if http_response.status_code in range(500, 600):
            ...
            if self.outage_start_time is not None:
                elapsed_outage_time = datetime.datetime.now(datetime.timezone.utc) - self.outage_start_time
    ...
    def __init__(self, config: Dict, dry_run: bool = False) -> None:
        ...
        # legacy logging config format
            ...
            if 'gotify' in logging_conf:
                ...
                queue_listener = QueueListener(log_queue, GotifyHandler(**logging_conf['gotify']))
        ...
        for time_delta_str in config['bid_sniper'].get('alert_time_deltas', list()):
            time_delta = cal.parseDT(time_delta_str, sourceTime=datetime.datetime.min)[0] - datetime.datetime.min
        ...
        self.scheduled_tasks = set()  # Will contain itemIds tentatively scheduled for actions
    ...
    def task_err_handler(self, finished_task: asyncio.Task) -> None:
        ...
        if coro_exception:
            self.logger.error(
                f'Exception in coroutine "{getattr(finished_task.get_coro(), "__name__", "null")}" - {type(coro_exception).__name__} - {coro_exception}'  # noqa: E501
            )
    ...
    async def time_alert(self, item_id: int, end_time: datetime.datetime) -> None:
        ...
        self.update_favorites_cache(self.config['bid_sniper'].get('favorites_max_cache_seconds', 60))
        ...
        if favorite:
            ...
            delta_until_end = end_time.replace(microsecond=0) - datetime.datetime.now(datetime.timezone.utc).replace(
                microsecond=0
            )
            self.logger.warning(f'Time alert - {favorite["title"]} ending in {delta_until_end}')
    ...
    async def place_bid(self, item_id: int) -> None:
            ...
            try:
                ...
                if bid_summary:
                    ...
                    if bidder_name in self.config.get('friend_list', list()):
                        self.logger.info(
                            "Canceling bid due to friendship for item '{favorite['title']}' - current high bidder {bidder_name}"  # noqa: E501
                        )
                        return None

            except BaseException as be:
                self.logger.error(f"{type(be).__name__} getting info for item ID '{item_id} - {be} - continuing")
        ...
        if not self.dry_run:
            ...
            try:
                self.bid_shopgoodwill_client.place_bid(item_id, max_bid, favorite['sellerId'], quantity=1)
    ...
    async def main_loop(self) -> None:
        ...
        while True:
            ...
            for item_id, favorite_info in self.favorites_cache['favorites'].items():
                ...
                if (end_time - min_scheduling_timedelta) <= now + datetime.timedelta(seconds=refresh_seconds * 3):
...
def parse_args():
    ...
    parser.add_argument(
        ...
        '--dry-run',
        action='store_true',
        help='If set, do not perform any actions that modify state on ShopGoodwill (eg. placing bids)',
    )
